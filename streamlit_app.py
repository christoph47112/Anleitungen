...
def generate_summary_and_steps(content):
    ...
    steps = "
".join([f"- Schritt {idx + 1}: {line.strip()}" for idx, line in enumerate(lines) if line.strip() and not line.isspace()])".join([f"- Schritt {idx + 1}: {line.strip()}" for idx, line in enumerate(lines) if line.strip() and not line.isspace()])
